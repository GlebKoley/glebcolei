...
const AviableTimeCard = ({ data }) => {
   const { availability_text, availability_subtext, not_availability_text } = data;
   const [isAvailable, setIsAvailable] = useState(null);

   const currentAviableText = !isAvailable ? not_availability_text : availability_text;
   ...
   return (
      <div className="bg-lightBlack mobile:flex-row mobile:p-5 mt-2 flex flex-col items-center gap-2 rounded-2xl border border-[#272727] p-2">
         ...
         <span
            ...
            dangerouslySetInnerHTML={{ __html: asHTML(availability_subtext) }}
         />
      </div>
   );
};
