...
interface CardProps {
   htmlLeftCardText: string;
   left_card_data: AboutBlockSlice['left_card_data'];
}

const AboutMeLeftCard = ({ left_card_data, htmlLeftCardText }: CardProps) => {
   return (
      <div className="mobile:hover:scale-[1.03] mobile:p-6 bg-lightBlack flex h-full flex-col gap-4 rounded-2xl px-3 py-4 transition-transform duration-300 ease-out">
         ...
         <div className="mobile:gap-3 grid h-full grid-cols-2 gap-2">
            {left_card_data.map(({ link, emoji, top_text, main_text }, index) => {
               const emojiText = asHTML(emoji);
