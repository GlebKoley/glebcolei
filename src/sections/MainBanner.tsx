...
import { MainBannerSlice } from '@/lib/types/slices';
import { createDownloadHref } from '@/lib/utils/createDownloadHref';

const MainBanner = ({ data }: { data: MainBannerSlice }) => {
   const { image, title, description, buttons_group } = data;
                  ...
                  {buttons_group.map(({ link, button_variant }) => {
                     return (
                        <Link
                           key={link.key}
                           href={link.uid ?? {}}
                           className={button_variant.data.variant}
